...
Natural-language captures can create reminders without a cloud service. Try `Remind me to call Mum in 20 minutes`, `Remind me to have coffee at 9pm`, `Remind me at 7.30 to have a coffee`, `Remind me to put the bins out tomorrow at 9`, or `Remind me to renew the pass on 2026-08-14 at 14:30`. Times may use a colon or full stop, and the time may come before or after the action. A bare time means the next occurrence of that time: today when it is still ahead, otherwise tomorrow. Index Inbox stores the action as a task and interprets calendar times using `REMINDER_TIMEZONE`. Deliberately unambiguous English phrases are supported; anything it cannot confidently parse remains an ordinary note.
